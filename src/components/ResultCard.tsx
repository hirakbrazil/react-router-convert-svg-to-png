...
const ResultCard = ({
  ...
  if (adjustForInflation) {
    ...
    let totalPresentValueOfWithdrawals = 0;
    for (let year = 0; year < timePeriod; year++) {
      const inflationFactor = Math.pow(1 + inflationRate / 100, year);
      const adjustedWithdrawal = monthlyWithdrawal * inflationFactor;
      const yearlyWithdrawals = adjustedWithdrawal * withdrawalsPerYear;
      
      // Calculate present value of this year's withdrawals
      const presentValue = yearlyWithdrawals / Math.pow(1 + inflationRate / 100, year);
      totalPresentValueOfWithdrawals += presentValue;
    }

    // Calculate future value of initial investment
    const futureValueOfInvestment = totalInvestment * Math.pow(1 + inflationRate / 100, timePeriod);

    // Calculate real value considering inflation
    const nominalFinalValue = futureValueOfInvestment - totalPresentValueOfWithdrawals;
    finalValue = Math.round(nominalFinalValue / Math.pow(1 + inflationRate / 100, timePeriod));
  }
  ...
  return (
    <div className="bg-card dark:bg-card rounded-xl shadow-lg p-6 space-y-4">
        ...
        <>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex flex-col items-end">
              <span className="text-xl font-semibold text-foreground">
                {formatCurrency(calculateLastSWP(monthlyWithdrawal, timePeriod, inflationRate), currency)}
              </span>
            </div>
          </div>
          
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {getFrequencyText(withdrawalFrequency)} expense after {timePeriod} years is {formatCurrency(finalValue, currency)}
          </div>
        </>
      )}
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-xl font-semibold text-foreground">
          {formatCurrency(totalWithdrawal + finalValue, currency)}
        </span>
      </div>
      <div className="flex justify-between items-center">
        <div className="flex flex-wrap items-center gap-x-1">
        ...
        <div className="flex flex-col items-end">
          <span className={`text-xl font-semibold ${finalValue > totalInvestment ? 'text-green-500 dark:text-green-400' : 'text-foreground'}`}>
            {formatCurrency(finalValue - totalInvestment, currency)}
          </span>
          <span className={`text-base font-medium ${finalValue > totalInvestment ? 'text-green-500 dark:text-green-400' : 'text-foreground'}`}>
            {((finalValue - totalInvestment) / totalInvestment * 100).toFixed(2)}%
          </span>
        </div>
      </div>
